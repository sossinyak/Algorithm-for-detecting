...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=["legacy", "core", "fast", "strong", "pruned", "tuned", "full", "research", "protocol"],
    )
    parser.add_argument(
        "--all_datasets",
        action="store_true",
        help="Run the comparison on every dataset under ./data that has split/A, split/B and split/label.",
    )
    parser.add_argument(
        "--synthetic",
        action="store_true",
        help="Generate a small synthetic laboratory dataset and run the algorithm on it.",
    )
    parser.add_argument("--synthetic_path", type=str, default="./data/synthetic-lab")
    parser.add_argument("--synthetic_val_pairs", type=int, default=3)
    parser.add_argument("--synthetic_test_pairs", type=int, default=6)
    ...
    if args.all_datasets:
        ...
        if not dataset_paths:
            raise RuntimeError(f"No datasets found under {data_root.resolve()}")
