...
def _save_comparison_visual(
    ...
    panels = [
        ...
        ("GT", _overlay_mask(pair["img_b"], pair["label"], (0, 255, 0)), "rgb"),
    ]
...
def _save_f1_plot(summary: dict[str, dict], output_path: Path) -> None:
    ...
    fig_width = max(10, min(28, 0.42 * len(method_names)))
    fig, ax = plt.subplots(figsize=(fig_width, 6))
    colors = plt.cm.tab20(np.linspace(0, 1, max(len(method_names), 1)))
    bars = ax.bar(method_names, f1_scores, color=colors)
...
def run_comparison_experiment(config: dict) -> dict:
    ...
    if not pairs:
        ...
        raise RuntimeError(f"No image pairs found in {Path(data_path) / split}")
    if not labeled_pairs:
        data_path = config.get("data", {}).get("data_path", "./data/LEVIR-CD-filtred")
        raise RuntimeError(f"No labeled pairs found in {Path(data_path) / split / 'label'}")

    # Сравниваем классические базовые подходы с основным адаптивным методом.
    method_set = str(experiment_cfg.get("method_set", "strong"))
