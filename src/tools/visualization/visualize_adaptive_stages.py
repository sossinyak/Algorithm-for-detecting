"""Визуализация промежуточных этапов Adaptive PCA-CVA для нескольких патчей."""
...
def _stage_rows(intermediate: dict, label: np.ndarray) -> list[dict]:
    ...
    stage_names = [
        ...
        ("closing_mask", "Closing"),
        ("area_mask", "Area filter"),
        ("filled_mask", "Fill holes"),
        ("final_mask", "Final"),
    ]
...
def _save_figure(
    ...
    panels = [
        ...
        ("GT overlay", _overlay_mask(img_b, label, (0, 255, 0)), "rgb"),
        ("Score-map", intermediate["change_map"], "viridis"),
        ("Threshold", intermediate["threshold_mask"], "gray"),
        ("Median", intermediate["median_mask"], "gray"),
        ("Opening", intermediate["opening_mask"], "gray"),
        ("Closing", intermediate["closing_mask"], "gray"),
        ("Area filter", intermediate["area_mask"], "gray"),
        ("Fill holes", intermediate["filled_mask"], "gray"),
        ("Final overlay", _overlay_mask(img_b, intermediate["final_mask"], (255, 0, 0)), "rgb"),
        ("Errors", _error_map(intermediate["final_mask"], label), "rgb"),
    ]
    ...
    plt.suptitle(
        (
            ...
            f"P={final_metrics['precision']:.3f} "
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Visualize stages of Adaptive PCA-CVA")
