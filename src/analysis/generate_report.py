...
def build_report(results_dir: Path, output_html: Path) -> Path:
    ...
    unique_images = list(dict.fromkeys(final_plots + plot_paths[:12]))
...
<body>
  ...
  <p>Отчет собран автоматически из артефактов экспериментов. Основная метрика сравнения: F1-score.</p>
  ...
  {_table(best, ["dataset", "method", "precision", "recall", "f1", "accuracy", "time_ms", "tune_split", "eval_split"])}

  <h2>Статистическая проверка</h2>
  {_table(stats)}

  <h2>Фрагмент parameter study</h2>
  {_table(trials, ["dataset", "method", "threshold", "sigma", "clahe_clip_limit", "clahe_tile_grid_size", "canny_low", "tune_precision", "tune_recall", "tune_f1"], max_rows=60)}

  <h2>Промежуточные этапы Adaptive PCA-CVA</h2>
  <p>Для выбранных test-патчей показаны исходные снимки, эталонная маска, score-map, пороговая маска, морфологическая обработка, финальная маска и карта ошибок.</p>
