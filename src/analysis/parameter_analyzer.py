"""Систематический parameter study для исследовательского протокола."""
...
from pipelines.baseline_methods import (
    CombinedChangePipeline,
    EdgeBasedDetector,
    build_tunable_classical_method,
    classical_score_functions,
)
...
def _method_from_params(params: dict, config: dict) -> object:
    ...
    if method_type == "classical":
        return build_tunable_classical_method(
            ...
            clahe_tile_grid_size=params.get("clahe_tile_grid_size", 16),
        )
    if method_type == "combined":
        return CombinedChangePipeline(
    ...
    if method_type == "edge":
        return EdgeBasedDetector(
            ...
            edge_weight=params.get("edge_weight", 0.45),
            diff_weight=params.get("diff_weight", 0.55),
            sigma=params.get("sigma", "auto"),
            threshold=params["threshold"],
            threshold_scale=params.get("threshold_scale", 1.0),
            adaptive_block_size=params.get("adaptive_block_size", 35),
...
def build_parameter_grid(image_area: int, config: dict) -> list[dict]:
        ...
        for threshold, scale in thresholds:
            for sigma in [None, "auto", 1.0]:
                ...
                if score_name == "CLAHE-CVA":
                    for clip_limit, tile_size in itertools.product([1.0, 2.0, 3.0], [8, 16, 32]):
                        rows.append({**base, "clahe_clip_limit": clip_limit, "clahe_tile_grid_size": tile_size})
                else:
                    rows.append(base)

    combined_weights = [
        {"AbsDiff": 0.35, "LogRatio": 0.25, "RGB-CVA": 0.40},
        {"AbsDiff": 0.25, "LogRatio": 0.25, "CLAHE-CVA": 0.50},
        {"LogRatio": 0.30, "LAB-CVA": 0.35, "EdgeDiff": 0.35},
    ]
    for weights in combined_weights:
        for threshold, scale in [("otsu", 0.9), ("otsu", 1.0), ("adaptive", 1.0)]:
            rows.append(
                {
                    "method": "Combined-AbsLogCVA",
    ...
    for low, high in [(30, 90), (50, 150), (80, 200)]:
        for threshold in ["otsu", "adaptive"]:
            rows.append(
                {
                    "method": "Edge-Canny-XOR",
                    "method_type": "edge",
                    "canny_low": low,
                    "canny_high": high,
                    "edge_weight": 0.45,
...
def build_monte_carlo_candidates(image_area: int, trials: int, seed: int) -> list[dict]:
    ...
    Monte Carlo полезен здесь не как замена сетке, а как способ проверить
    промежуточные комбинации: другие веса combined-map, локальные пороги,
    min_area, sigma и параметры морфологии.
    """
    if trials <= 0:
        return []
    rng = random.Random(seed)
    score_names = list(classical_score_functions())
    min_area_low = max(8, int(image_area * 0.0004))
    min_area_high = max(32, int(image_area * 0.004))
    candidates: list[dict] = []
    for trial in range(trials):
        method_family = rng.choices(
            ["classical", "combined", "edge", "adaptive_pca"],
        ...
        if method_family == "classical":
            ...
            if score_name == "CLAHE-CVA":
                row.update(
                    clahe_clip_limit=round(rng.uniform(1.0, 3.5), 2),
                    clahe_tile_grid_size=rng.choice([8, 12, 16, 24, 32]),
                )
            candidates.append(row)
        elif method_family == "combined":
            weights = {
                "AbsDiff": round(rng.random(), 3),
                "LogRatio": round(rng.random(), 3),
                rng.choice(["RGB-CVA", "LAB-CVA", "HSV-CVA", "CLAHE-CVA"]): round(rng.random(), 3),
            }
            candidates.append(
                {
                    "method": "Combined-AbsLogCVA",
                    ...
                    "weights": weights,
                    "clahe_clip_limit": round(rng.uniform(1.0, 3.5), 2),
                    "clahe_tile_grid_size": rng.choice([8, 16, 32]),
                    **common,
                }
            )
        elif method_family == "edge":
            low = rng.choice([20, 30, 40, 50, 70, 90])
            high = rng.choice([90, 120, 150, 180, 220])
            candidates.append(
                {
                    "method": "Edge-Canny-XOR",
                    "method_type": "edge",
                    "canny_low": min(low, high - 10),
                    "canny_high": high,
                    "edge_weight": round(rng.uniform(0.25, 0.75), 3),
                    "diff_weight": round(rng.uniform(0.25, 0.75), 3),
                    **common,
                }
            )
        else:
...
def _save_parameter_plots(trials: pd.DataFrame, output_dir: Path) -> None:
    ...
    for column in ["sigma", "threshold", "threshold_scale", "clahe_clip_limit", "clahe_tile_grid_size", "canny_low", "adaptive_block_size"]:
        ...
        ax.bar(grouped[column].astype(str), grouped["tune_f1"], color="#4C78A8")
        ax.set_title(f"Средний tune F1 по параметру {column}")
...
def main() -> None:
    ...
    try:
        ...
        if not summaries:
            raise RuntimeError("Не найдено датасетов для parameter study.")
